...
* What alerts are: Alerts are operational signals raised when rules detect conditions that need operator attention (e.g. long queues, runway hazards, low grip, tamper, gate conflicts). They are stored in the DB and returned in GET /alerts and in the overview snapshot (active_alerts). The system suggests actions; it does not execute them. Operators resolve alerts after taking (or deciding not to take) action.
* How they are generated: On each GET /overview (and GET /aodb/overview), the route runs run_all_intelligence(db) before building the overview. Intelligence runs rule functions in services/intelligence.py that read data via crud (passenger_flow, runways, infrastructure_assets, flights) and call crud.create_alert(...) when a condition is met. Each rule passes a stable uniqueness_key so the same condition does not create duplicate unresolved alerts.
* Where rules live: services/intelligence.py. Functions: run_queue_alerts (security_queue_count above SECURITY_QUEUE_THRESHOLD), run_runway_hazard_alerts (hazard_detected or grip_score below GRIP_SCORE_LOW_THRESHOLD), run_tamper_alerts (tamper_detected on infrastructure), run_gate_conflict_alerts (two flights same gate with overlapping times). run_all_intelligence(db) runs all four.
* Deduplication with uniqueness_key: In crud.create_alert, (1) if uniqueness_key is set, the code looks up an unresolved alert with that key (get_unresolved_alert_by_uniqueness_key). (2) If not found, it looks up by (alert_type, related_entity_type, related_entity_id) via get_unresolved_alert_by_entity (fallback for legacy rows with null key). If either finds a row, create_alert returns None and no row is inserted. So at most one unresolved alert per logical condition.
* Resolution vs solving: Marking an alert “resolved” means the operator has acknowledged it (e.g. took action or decided to defer). It does not automatically fix the real-world condition. If the condition persists, the next run of intelligence can create a new alert for the same logical condition once the previous one is resolved (no unresolved duplicate exists anymore).
* Alerts can reappear: Because resolution only flips the resolved flag, the same situation (e.g. queue still high, grip still low) will trigger a new alert on a later /overview request if the rule still sees the condition. That is by design: persistent conditions keep surfacing until the underlying issue is addressed.
* Suggested actions: Each alert type has an operator-friendly suggested_action (concise, realistic). Suggested actions are computed at serialization time from alert_type (schemas.ALERT_SUGGESTED_ACTIONS / get_suggested_action), not stored in the DB. The API includes suggested_action in AlertResponse; the UI shows it in the alerts table. The system suggests only; it does not execute any action.
* Alert types and suggested actions:
  * queue — Security queue count high. Suggested: Deploy extra security lanes or redirect passengers to reduce queue depth; monitor wait times.
  * runway_hazard — Hazard detected on runway. Suggested: Inspect runway and clear hazard; consider temporary closure until cleared.
  * grip (low_grip) — Low runway grip score. Suggested: Schedule runway surface treatment or restrict operations until grip improves.
  * security (tamper) — Infrastructure tamper detected. Suggested: Verify asset integrity and secure area; escalate to security if tamper confirmed.
  * gate_conflict — Two flights overlap at same gate. Suggested: Reassign gate for one of the flights or adjust schedule to resolve overlap.
